...
export const ossMovieApiProject: Project = {
  ...
  // Background
  background: [
    "*조교 역할로 전공 수업에서 OpenAPI 기반 React 실습이 필요*",
    "*리액트를 처음 경험하는 학생들이 복제·확장 가능한 구조가 필요*",
  ],

  // Key Features
  keyFeatures: [
    "*Open API 연동 패턴 예시(KMDB/KOBIS)*",
    "*검색 · 목록 · 상세 흐름으로 구성된 반응형 UI*",
  ],


  // Stack
  stackDetail: [
    "*OpenAPI*: OpenAPI를 사용해 데이터를 가져오는 수업 목표에 적합",
  ...
  // Result
  results: [
    "*수업 실습에서 모든학생이 다양한 OpenAPI 연동에 성공, CORS 이슈 없이 학습 진행*",
    "*프록시/환경변수/라우팅 사례를 쉽게 재사용*",
    "*배포 전 로컬환경에서 Open API를 안전하게 다루는 방법 학습*",
  ],

  problemSolving: [
    {
      problem: "**학생들이 배포 전 로컬환경에서 OpenAPI를 호출하면 대부분 CORS 에러로 데이터 수신이 안 됨**",
      cause: "브라우저의 **Same-Origin Policy**로 인해 타 도메인 요청에 사전 요청(Preflight)과 CORS 헤더 검증이 필요한데, 공개 API가 브라우저 직접 호출을 허용하지 않는 경우가 많음",
      ...
      alternatives: [
        {
          ...
          description: [
            "로컬 개발에서만 프록시를 설정해 브라우저가 같은 출처로 인식",
            "요청 경로에 접두사를 두고 프록시가 외부 API로 전달",
          ],
          pros: [
            "설정이 단순하고 교육용 예제에 적합",
            "클라이언트 코드 변경 최소화"
